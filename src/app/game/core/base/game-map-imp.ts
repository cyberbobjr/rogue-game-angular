...
export class GameMapImp implements GameMap {
    ...
    private _entryPosition: Position2D;
    private _exitPosition: Position2D;
    ...
    get entryPosition(): Position2D {
        return this._entryPosition;
    }

    set entryPosition(value: Position2D) {
    ...
    set exitPosition(value: Position2D) {
    ...
    public computeLOSMap(mainActor: Entity): GameMap {
        const mainActorPosition: Position2D = mainActor.getPosition();
    ...
    public getLosForPosition(position: Position2D): number {
        return this._losMap[position.y][position.x];
    }

    public getTilesAround(position: Position2D): Array<Array<Tile>> {
        return this._extract<Tile>(this._data, position.x - 1, position.y - 1, 3, 3);
    }

    public getTileAt(position: Position2D): Tile {
        return this.getDataAt(position.x, position.y) as Tile;
    }

    /**
     * Return all position for Tile
     */
    // @ts-ignore
    public getAllPosition<T>(ctor: { new(...args: any[]): T }): Array<Position2D> {
        const allPositions: Array<Position2D> = [];
        this._data.forEach((rows: Array<Iobject>) => {
            rows.forEach((column: Iobject) => {
                if (column instanceof ctor) {
                    const tile: Tile & T = (column as Tile & T);
                    allPositions.push(tile.position.clone());
                }
            });
        });
        return allPositions;
    ...
    public getFreePositionForRoom(roomNumber: number): Position2D | null {
    ...
    public getNextPosition(originPosition: Position2D, destPosition: Position2D): Position2D | null {
        const astar: AStar = new Path.AStar(destPosition.x, destPosition.y, (x: number, y: number) => {
            const info: Iobject = this.getTileAt(new Position2D(x, y));
    ...
    public isPositionVisible(position: Position2D): boolean {

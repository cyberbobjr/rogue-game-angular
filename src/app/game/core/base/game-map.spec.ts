...
describe('Game map', () => {
  ...
  it('will check for generic Tile', () => {
    const chestPosition: Position2D = new Position2D(5, 5);
    const gameMap: GameMapImp = new GameMapImp(10, 10);
    const chestTile: ChestTile = new ChestTile(chestPosition);
    gameMap.setTile(chestTile);
    const testingArray: Array<Position2D> = gameMap.getAllPosition<ChestTile>(ChestTile);
  ...
  it('should extract tile from map', () => {
    const gameMap: GameMapImp = new GameMapImp(10, 10);
    const tile: Tile = TilesFactory.createTile(TileType.FLOOR, new Position2D(5, 5));

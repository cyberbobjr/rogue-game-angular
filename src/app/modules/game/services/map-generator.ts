...
export class MapGenerator {
  ...
  generateNewMap(level = 1): GameMap<Iobject> {
    const map: GameMap<Iobject> = this._generateMap(80, 80, Math.round(Math.random() * 100), level);
  ...
  private _getEntryPosition(): Position {
    const rooms: Array<Room> = this._getRooms();
    const room: Room = rooms[0];
    return this._getRoomCenter(room);
  }

  private _getExitPosition(): Position {
    const rooms: Array<Room> = this._getRooms();
    const room: Room = rooms[0];
    return this._getRoomCenter(room);
  }
  ...
  private _generateMap(width: number, height: number, seed = 511, level = 1): GameMap<Iobject> {
    ...
    this._generateNextLevelAccess(map, this._rotEngine);
    this._generatePreviousLevelAccess(map, this._rotEngine);
    return map;
  }
  ...
  private _generateNextLevelAccess(map: GameMap<Iobject>, rotMap: Digger) {
  ...
  private _generatePreviousLevelAccess(map: GameMap<Iobject>, rotMap: Digger) {
    const rooms: Array<Room> = rotMap.getRooms();
